...
test('should handle error state', async () => {
    renderWithMockProvider(<OrganizationTasksPage />, {
        ...
        error: new Error('Oooops! Something went wrong'),
    });
    // Wait for the application to update
    await act(async () => new Promise(resolve => setTimeout(resolve, 0)));
    expect(screen.getByText(/Oooops! Something went wrong/i)).toBeInTheDocument();
});
...
test('should load organization tasks', async () => {
    renderWithMockProvider(<OrganizationTasksPage />, {
        ...
        data: {
            organizationTasks: [
                { id: 1, title: 'Vet for Murkė', description: 'Place: Kaunakiemio g. 15a', isDone: false },
            ],
        },
    });
    // Wait for the application to update
